...
BEGIN
    ...
    -- Check category_type.namekh
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'category_type' AND column_name = 'namekh'
    ) THEN
        issues := array_append(issues, 'category_type.namekh missing');
    END IF;
    
    -- Check category_type.namejp
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'category_type' AND column_name = 'namejp'
    ) THEN
        issues := array_append(issues, 'category_type.namejp missing');
    END IF;
...
-- ============================================================================
-- 6.x Verify category_type translations
-- ============================================================================
\echo '🌐 Step 6.x: Verifying category_type translations...'
...
BEGIN
    ...
    FROM category_type
    WHERE
        (name = 'Product & Feature' AND (namekh IS DISTINCT FROM 'ផលិតផល និងលក្ខណៈពិសេស' OR namejp IS DISTINCT FROM '製品・機能'))
        OR (name = 'Event'          AND (namekh IS DISTINCT FROM 'ព្រឹត្តិការណ៍' OR namejp IS DISTINCT FROM 'イベント'))
        OR (name = 'News'           AND (namekh IS DISTINCT FROM 'ព័ត៌មាន' OR namejp IS DISTINCT FROM 'ニュース'))
        OR (name = 'Other'          AND (namekh IS DISTINCT FROM 'ផ្សេងៗ' OR namejp IS DISTINCT FROM 'その他'));

    IF mismatch_count > 0 THEN
        RAISE EXCEPTION '❌ category_type translations mismatch for % rows', mismatch_count;
    ELSE
        RAISE NOTICE '✅ category_type translations look correct for default categories';
    END IF;
END$$;
...
BEGIN
    ...
    -- Check category_type.namekh is varchar(255)
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        ...
        issues := array_append(issues, 'category_type.namekh should be VARCHAR(255)');
    END IF;

    -- Check category_type.namejp is varchar(255)
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'category_type'
        ...
        issues := array_append(issues, 'category_type.namejp should be VARCHAR(255)');
    END IF;
    
    IF array_length(issues, 1) IS NULL THEN
        RAISE NOTICE '✅ All column types are correct';
    ELSE
        RAISE EXCEPTION '❌ Type issues: %', array_to_string(issues, ', ');
    END IF;
END$$;
