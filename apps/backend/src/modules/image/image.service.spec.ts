...
describe('ImageService', () => {
  ...
  const createMockBuffer = (size = 1024): Buffer =>
    Buffer.alloc(size, 'test-data');
  beforeEach(async () => {
    const mockImageRepo = {
      findOne: jest.fn(),
  ...
  describe('findByFileId', () => {
    ...
    describe('Error Cases', () => {
      it('should throw NotFoundException when image not found', async () => {
        ...
        await expect(service.findByFileId(fileId)).rejects.toThrow(
          NotFoundException
        );
        try {
          ...
          expect(error.response.responseMessage).toBe(
  ...
  describe('create', () => {
    ...
    describe('Success Cases', () => {
      ...
      it('should handle PNG with alpha channel', async () => {
        ...
        const mockSharpInstance = {
          metadata: jest
            .fn()
            .mockResolvedValue({ width: 100, height: 100, hasAlpha: true }),
          resize: jest.fn().mockReturnThis(),
          png: jest.fn().mockReturnThis(),
          jpeg: jest.fn().mockReturnThis(),
          toBuffer: jest.fn().mockResolvedValue(createMockBuffer(512)),
    ...
    describe('Race Condition Handling', () => {
      it('should handle unique constraint violation on fileHash (race condition)', async () => {
        ...
        const raceConditionError: any = new Error(
    ...
    describe('Aspect Ratio Handling', () => {
      ...
      it('should handle PNG with alpha channel and maintain aspect ratio', async () => {
        ...
        const mockSharpInstance = {
          metadata: jest
            .fn()
            .mockResolvedValue({ width: 2000, height: 1000, hasAlpha: true }), // 2:1 PNG
          resize: jest.fn().mockReturnThis(),
          png: jest.fn().mockReturnThis(),
          jpeg: jest.fn().mockReturnThis(),
          toBuffer: jest.fn().mockResolvedValue(createMockBuffer(2000)),
  ...
  describe('buildImageUrl', () => {
    ...
    it('should enforce HTTPS for production domains', () => {
      ...
      const url2 = service.buildImageUrl(
        fileId,
        null,
        'http://bakong-notification.com'
      );
  ...
  describe('validateImageUrl', () => {
    ...
    it('should build URL even with invalid fileId format (validation happens elsewhere)', () => {
      ...
      const url = service.validateImageUrl(
        'https://example.com',
        invalidFileId
      );
