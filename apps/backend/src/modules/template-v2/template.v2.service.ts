...
import { TemplateV2 } from 'src/entities/template.v2.entity'
...
@Injectable()
export class TemplateServiceV2 implements OnModuleInit {
  private readonly logger = new Logger(TemplateServiceV2.name)
  constructor(
    @InjectRepository(TemplateV2) private readonly repo: Repository<TemplateV2>,
    @InjectRepository(TemplateTranslationV2)
    private readonly translationRepo: Repository<TemplateTranslationV2>,
    @InjectRepository(Image)
    private readonly imageRepo: Repository<Image>,
    @Inject(forwardRef(() => NotificationServiceV2))
    public readonly notificationService: NotificationServiceV2,
    private readonly schedulerRegistry: SchedulerRegistry,
    private readonly imageService: ImageService,
    private readonly baseFunctionHelperV2: BaseFunctionHelperV2,
  ...
  async update(id: number, dto: UpdateTemplateDtoV2, currentUser?: any, req?: any) {
    const {
      platforms,
      bakongPlatform,
      translations,
  ...
  async editPublishedNotification(id: number, dto: UpdateTemplateDtoV2, currentUser?: any, req?: any) {
  ...
  private formatTemplateResponse(template: TemplateV2, req?: any) {
    ...
    const baseUrl = this.baseFunctionHelperV2
      ? this.baseFunctionHelperV2.getBaseUrl(req)
      : 'http://localhost:4005'
    ...
    const ct = template.categoryTypeEntity
    ...
    const formattedTemplate: any = {
      ...
      notificationType: template.notificationType,
      categoryType:
        ct && language === Language.KM && ct.namekh ? ct.namekh :
          ct && language === Language.JP && ct.namejp ? ct.namejp :
            ct?.name,
      ...
      translations: template.translations
        ? template.translations.map((translation) => ({
          ...
          linkPreview: translation.linkPreview,
          image: translation.imageId
            ? {
              fileId: translation.imageId,
              mimeType: translation.imageId
                ? translation.imageId.substring(0, 100)
                : null,
              originalFileName: translation.imageId
                ? translation.imageId.substring(0, 100)
                : null,
            }
            : translation.imageId
  ...
  validateModificationTemplate(template: TemplateV2, allowDelete = false) {
   ...
   */
  private async validateMatchingUsers(template: TemplateV2): Promise<boolean> {
    try {
      // Parse platforms
      ...
      const tempTemplate = { ...template } as TemplateV2
  ...
  addScheduleNotification(template: TemplateV2) {
  ...
  addIntervalNotification(template: TemplateV2) {
  ...
  async findTemplateById(templateId: string): Promise<TemplateV2> {
    const template = await this.repo.findOne({
  ...
  ): Promise<{ template: TemplateV2; translation: TemplateTranslationV2 } | null> {
  ...
  public findBestTranslation(template: TemplateV2, language?: string): TemplateTranslationV2 | null {
