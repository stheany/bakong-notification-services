...
describe('BaseFunctionHelper - inferBakongPlatform', () => {
  ...
  describe('inferBakongPlatform from participantCode', () => {
    ...
    it('should infer BAKONG_JUNIOR from BKJR prefix', () => {
      expect(testInference('BKJRKHPPXXX', 'user@test')).toBe(
        BakongApp.BAKONG_JUNIOR
      );
      expect(testInference('bkjrkhppxxx', 'user@test')).toBe(
        BakongApp.BAKONG_JUNIOR
      ); // lowercase
      expect(testInference('BKJR123', 'user@test')).toBe(
        BakongApp.BAKONG_JUNIOR
      );
    });
    it('should infer BAKONG_TOURIST from TOUR prefix', () => {
      expect(testInference('TOURKHPPXXX', 'user@test')).toBe(
        BakongApp.BAKONG_TOURIST
      );
      expect(testInference('tourkhppxxx', 'user@test')).toBe(
        BakongApp.BAKONG_TOURIST
      ); // lowercase
      expect(testInference('TOUR123', 'user@test')).toBe(
        BakongApp.BAKONG_TOURIST
      );
    });
  ...
  describe('inferBakongPlatform from accountId', () => {
    ...
    it('should infer BAKONG_JUNIOR from @bkjr domain', () => {
      expect(testInference(undefined, 'user@bkjr')).toBe(
        BakongApp.BAKONG_JUNIOR
      );
      expect(testInference(undefined, 'user@BKJR')).toBe(
        BakongApp.BAKONG_JUNIOR
      ); // uppercase
      expect(testInference(undefined, 'test.user@bkjr')).toBe(
        BakongApp.BAKONG_JUNIOR
      );
    });
    it('should infer BAKONG_TOURIST from @tour domain', () => {
      expect(testInference(undefined, 'user@tour')).toBe(
        BakongApp.BAKONG_TOURIST
      );
      expect(testInference(undefined, 'user@TOUR')).toBe(
        BakongApp.BAKONG_TOURIST
      ); // uppercase
      expect(testInference(undefined, 'test.user@tour')).toBe(
        BakongApp.BAKONG_TOURIST
      );
    });
