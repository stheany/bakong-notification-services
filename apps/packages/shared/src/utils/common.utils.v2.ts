...
export interface ValidationResultV2<T> {
  isValid: boolean
...
export class ValidationUtilsV2 {
  ...
  static validateLanguage(value: string): ValidationResultV2<Language> {
  ...
  static validatePlatform(value: string): ValidationResultV2<Platform> {
    if (!value || typeof value !== 'string') {
      return {
        isValid: false,
        normalizedValue: null,
  ...
  static validateNotificationType(value: string): ValidationResultV2<NotificationType> {
    if (!value || typeof value !== 'string') {
      return {
        isValid: false,
        normalizedValue: null,
  ...
  static validateUserRole(value: string): ValidationResultV2<UserRole> {
    if (!value || typeof value !== 'string') {
      return {
        isValid: false,
        normalizedValue: null,
  ...
  static validateSendType(value: string): ValidationResultV2<SendType> {
    if (!value || typeof value !== 'string') {
      return {
        isValid: false,
        normalizedValue: null,
...
export const passwordFormatV2 = (password: string, required: boolean = false): boolean | string => {
  return ValidationUtilsV2.validatePassword(password, required)
}

export const emailFormatV2 = (email: string, required: boolean = false): boolean | string => {
  return ValidationUtilsV2.validateEmail(email, required)
}

export const usernameFormatV2 = (username: string, required: boolean = false): boolean | string => {
  return ValidationUtilsV2.validateUsername(username, required)
}

export const getErrorMessageV2 = (errorCode: number): string => {
...
export const ERROR_MESSAGESV2: Record<number, string> = (() => {
