...
export class DateUtils {
  ...
  static createCambodiaDate(
    ...
    return new Date(
      Date.UTC(utcYear, utcMonth - 1, utcDay, utcHours, minutes, seconds)
    );
  }
  static getTimezoneOffsetString(): string {
    return 'UTC+7 (Cambodia Time)';
  }
  static formatCambodiaDate(
    date: Date | string,
  ...
  private static convertToKhmerNumbers(arabicNumber: string): string {
    ...
    return arabicNumber.replace(
      /\d/g,
      (digit) => khmerNumbers[parseInt(digit)]
    );
  }
  ...
  static formatUTCToCambodiaDateTime(utcDate: Date | string): {
    date: string;
    time: string;
  } {
    const date = new Date(utcDate);
  ...
  static validateScheduleDate(dateStr: string): {
    isValid: boolean;
    error?: string;
  } {
    if (!dateStr) {
  ...
  static validateScheduleTime(timeStr: string): {
    isValid: boolean;
    error?: string;
  } {
    if (!timeStr) {
