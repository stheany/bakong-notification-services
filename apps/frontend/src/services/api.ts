      ...
      if (!isPublicEndpoint) {
        ...
        if (token && token.trim() !== '') {
          ...
          if (tokenParts.length === 3) {
            ...
            clearAuthState();
            return Promise.reject(
              new Error('Invalid token format. Please login again.')
            );
          }
        } else {
          ...
          clearAuthState();
          return Promise.reject(
            new Error('No authentication token found. Please login.')
          );
        }
      }
...
const addResponseInterceptor = (axiosInstance: typeof api) => {
  axiosInstance.interceptors.response.use(
    ...
    async (error) => {
      ...
      const isChangePasswordEndpoint = error.config?.url?.includes(
        '/auth/change-password'
      );
      const isTemplateEndpoint =
        error.config?.url?.includes('/template/create') ||
        (error.config?.url?.includes('/template/') &&
          error.config?.url?.includes('/update-publish')) ||
        (error.config?.url?.includes('/template/') &&
          error.config?.url?.includes('/remove'));
