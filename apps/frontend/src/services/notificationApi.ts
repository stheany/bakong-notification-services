...
export const notificationApi = {
          ...
          .map((template: any) => {
            ...
            return {
              ...
              image: translation.image
                ? `/api/v1/image/${translation.image.fileId}`
                : '',
  ...
  async updateNotification(
    ...
    const response = await api.post(
      `/api/v1/template/${id}/update-publish`,
      notification
    );
    return response.data;
  },
  ...
  async deleteNotification(id: number): Promise<void> {
    try {
      const response = await api.post(`/api/v1/template/${id}/remove`);
  ...
  async sendNotification(
    ...
    try {
      ...
      console.error(
        `Error sending notification with templateId ${templateId}:`,
        error
      );
      throw error;
    }
