  ...
  import type { NotificationV2 } from '@/services/notificationApi-v2'
  ...
  const emit = defineEmits<{
    ...
    publish: [notification: NotificationV2]
  }>()
  ...
  const hasValidImage = (notification: NotificationV2) => {
    return (
      notification.image &&
  ...
  const handleDeleteClick = async (notification: NotificationV2) => {
    const confirmed = await showDeleteDialog('notification')
    if (confirmed) {
      emit('delete', notification.id as number)
    }
  }

  const handlePublishClick = (notification: NotificationV2) => {
    emit('publish', notification)
  }

  const handleEditClick = (notification: NotificationV2) => {
