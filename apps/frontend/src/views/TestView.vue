  ...
  <div class="test-page">
    <div class="test-container">
      ...
      <p class="page-description">
        Test and validate FCM tokens, sync user data, and verify system
        functionality.
      </p>
      ...
      <el-tabs v-model="mainActiveTab" class="main-tabs">
        <el-tab-pane label="🔑 Test FCM Token" name="token">
          <div class="test-section">
            ...
            <p class="section-description">
              Test if an FCM token is valid. This will send a test notification
              to verify the token.
            </p>
            ...
            <div class="platform-info-box">
              ...
              <div class="info-content">
                ...
                <p>
                  In <strong>development</strong> environment, all platforms
                  (BAKONG, BAKONG_JUNIOR, BAKONG_TOURIST) use the same Firebase
                  project. Therefore, any valid token from that project will
                  work regardless of which platform you select. In
                  <strong>production/SIT</strong>, each platform has its own
                  Firebase project, so platform selection matters.
                </p>
              </div>
            </div>
              ...
              <el-form :model="tokenTestForm" label-width="140px">
                ...
                <el-form-item label="Platform">
                  ...
                  <div class="input-hint">
                    ℹ️ Platform selection determines which Firebase project to
                    use (only matters in production/SIT)
                  </div>
                </el-form-item>
                <el-form-item>
                  <el-button
                    ...
                    :disabled="
                      !tokenTestForm.token || !tokenTestForm.token.trim()
                    "
                    size="large"
                    class="test-token-btn"
                  >
                    <el-icon v-if="!testingToken" style="margin-right: 8px"
                      ><Check
                    /></el-icon>
            ...
            <div v-if="tokenTestResult" class="test-result">
              ...
              <div class="result-content">
                <div class="result-item">
                  ...
                  <el-tag
                    :type="tokenTestResult.formatValid ? 'success' : 'danger'"
                  >
                ...
                <div class="result-item">
                  ...
                  <el-tag
                    :type="tokenTestResult.firebaseValid ? 'success' : 'danger'"
                  >
                ...
                <div class="result-item">
                  ...
                  <el-tag
                    :type="tokenTestResult.isValid ? 'success' : 'danger'"
                    size="large"
                  >
                    {{
                ...
                <div v-if="tokenTestResult.messageId" class="result-item">
                  ...
                  <span class="result-value">{{
                    tokenTestResult.messageId
                  }}</span>
                </div>
                <div
                  v-if="tokenTestResult.error"
        ...
        <el-tab-pane label="🔄 Sync All Users" name="sync">
          <div class="test-section">
            ...
            <p class="section-description">
              Manually sync all user data from the database. This normalizes
              user fields, validates tokens, updates platform/language
              information, and tracks sync status.
            </p>
            ...
            <div class="sync-info-box" style="margin-top: 20px">
              ...
              <div class="info-content">
                ...
                <p>
                  Each user sync now tracks:
                  <code>status</code> (SUCCESS/FAILED),
                  <code>lastSyncAt</code> (timestamp), and
                  <code>lastSyncMessage</code> (success message or error
                  details). Check the database to see syncStatus JSONB column.
                </p>
              </div>
            </div>
            ...
            <div class="sync-info-box">
              <h3 class="info-box-title">
                When does user sync happen automatically?
              </h3>
              <ul class="sync-scenarios">
                <li>
                  <strong>1. When mobile app calls API endpoints:</strong>
                  <p>
                    <strong>✅ Syncs individual user data:</strong> When mobile
                    app calls <code>/notification/send</code> or
                    <code>/notification/inbox</code>, that specific user's data
                    (FCM token, platform, language) is synced immediately.
                  </p>
                  <p class="warning-note">
                    ⚠️ <strong>Important:</strong> If mobile app only receives
                    FCM push notifications without calling any API, user data
                    won't be synced. The mobile app must call at least one API
                    endpoint to update their data.
                  </p>
                </li>
                <li>
                  <strong>2. When sending notifications via template:</strong>
                  <p>
                    Before sending notifications, the system automatically
                    normalizes all existing user data in the database (platform,
                    language, token format validation). This only works with
                    data already in the database - it doesn't fetch new data
                ...
                <li>
                  ...
                  <p>
                    You can manually trigger a sync to normalize all user data,
                    validate tokens, and update platform/language fields. This
                    also only works with existing database data.
                  </p>
                </li>
              </ul>
              ...
              <div class="sync-improvement-box">
                ...
                <p>
                  The backend now
                  <strong>automatically cleans up invalid tokens</strong> when
                  FCM sends fail:
                </p>
                <ul class="auto-cleanup-list">
                  <li>
                    When sending notifications fails with invalid token errors,
                    the backend automatically clears those tokens
                  </li>
                  <li>
                    When syncing users, the backend automatically removes tokens
                    that are too short or malformed
                  </li>
                  <li>
                    This helps keep the database clean even if mobile apps don't
                    call APIs frequently
                  </li>
                </ul>
                <p class="note-text">
                  <strong>Note:</strong> While backend auto-cleanup helps
                  maintain data quality, mobile apps should still call
                  <code>/notification/send</code> or
                  <code>/notification/inbox</code> to sync NEW tokens and
                  updated user information (platform, language). Backend cleanup
                  only removes invalid tokens - it doesn't fetch new data from
                  external sources.
                </p>
              </div>
            </div>
            ...
            <div class="sync-actions">
              ...
              >
                <el-icon v-if="!syncingUsers" style="margin-right: 8px"
                  ><Check
                /></el-icon>
            ...
            <div v-if="syncResult" class="test-result">
              ...
              <div class="result-content">
                ...
                <div class="result-item">
                  ...
                  <el-tag
                    :type="syncResult.updatedCount > 0 ? 'success' : 'info'"
                  >
                ...
                <div class="result-item">
                  ...
                  <el-tag type="warning">{{
                    syncResult.platformUpdates
                  }}</el-tag>
                </div>
                <div class="result-item">
                  <span class="result-label">Language Updates:</span>
                  <el-tag type="warning">{{
                    syncResult.languageUpdates
                  }}</el-tag>
                </div>
                <div class="result-item">
                  <span class="result-label">Invalid Tokens:</span>
                  <el-tag
                    :type="syncResult.invalidTokens > 0 ? 'danger' : 'success'"
                  >
                ...
                <div
                  v-if="
                    syncResult.updatedIds && syncResult.updatedIds.length > 0
                  "
                  class="result-item"
                >
                  ...
                  <div class="updated-ids-list">
                    ...
                    <span
                      v-if="syncResult.updatedIds.length > 10"
                      class="more-ids"
                    >
        ...
        <el-tab-pane label="📬 Test Inbox API" name="inbox">
          <div class="test-section">
            ...
            <p class="section-description">
              Test the /inbox endpoint with two flows: Sync Data (when page/size
              are null) and Notification Center (when page/size are provided).
            </p>
            ...
            <div class="inbox-info-box">
              ...
              <ul class="sync-scenarios">
                <li>
                  ...
                  <p>
                    When <code>page</code> and <code>size</code> are
                    <code>null</code> or not provided, the API syncs user data
                    and returns a simple sync response.
                  </p>
                </li>
                <li>
                  <strong>2. Notification Center Flow:</strong>
                  <p>
                    When <code>page</code> and <code>size</code> are provided,
                    the API returns paginated notifications (existing behavior).
                  </p>
                </li>
              </ul>
            </div>
            ...
            <el-tabs v-model="inboxActiveTab" class="inbox-tabs">
                  ...
                  <el-form :model="inboxSyncForm" label-width="140px">
                    ...
                    <el-form-item label="Bakong Platform" required>
                      ...
                      >
                        ...
                        />
                        <el-option
                          label="BAKONG_TOURIST"
                          value="BAKONG_TOURIST"
                        />
                      </el-select>
                    </el-form-item>
                    <el-form-item>
                      ...
                      <div class="input-hint" style="margin-top: 8px">
                        💡 <strong>Test Failure:</strong> Click "Test Failure
                        Scenario" to simulate a sync failure (uses invalid
                        accountId format to trigger database error)
                      </div>
                    </el-form-item>
                  </el-form>
                </div>
              ...
              <el-tab-pane
                label="📋 Notification Center Flow"
                name="notification"
              >
                <div class="inbox-form-container">
                  <el-form :model="inboxNotificationForm" label-width="140px">
                    ...
                    <el-form-item>
                      ...
                      >
                        <el-icon
                          v-if="!testingInboxNotification"
                          style="margin-right: 8px"
                          ><Check
                        /></el-icon>
                        {{
                          testingInboxNotification
                            ? 'Loading...'
                            : 'Test Notification Center Flow'
                        }}
                      </el-button>
                    </el-form-item>
                  </el-form>
            ...
            <div v-if="inboxSyncResult" class="test-result">
              ...
              <div class="result-content">
                <div class="result-item">
                  ...
                  <el-tag
                    :type="
                      inboxSyncResult.responseCode === 0 ? 'success' : 'danger'
                    "
                  >
                ...
                <div class="result-item">
                  ...
                  <span class="result-value">{{
                    inboxSyncResult.responseMessage
                  }}</span>
                </div>
                <div
                  v-if="
                    inboxSyncResult.responseCode === 0 && inboxSyncResult.data
                  "
                  class="result-item"
                >
                  <span class="result-label">Account ID:</span>
                  <span class="result-value">{{
                    inboxSyncResult.data.accountId
                  }}</span>
                </div>
                <div
                  v-if="
                ...
                >
                  ...
                  <el-tag type="info">{{
                    inboxSyncResult.data.bakongPlatform
                  }}</el-tag>
                </div>
                <div
                  v-if="
                ...
                >
                  ...
                  <span class="result-value">{{
                    inboxSyncResult.data.syncedAt
                  }}</span>
                </div>

                <div
                  v-if="
                    inboxSyncResult.responseCode === 0 &&
                    inboxSyncResult.data?.syncStatus
                  "
                  class="result-item"
                >
                  ...
                  <div class="sync-status-details">
                    <el-tag
                      :type="
                        inboxSyncResult.data.syncStatus.status === 'SUCCESS'
                          ? 'success'
                          : 'danger'
                      "
                      style="margin-right: 8px"
                    ...
                    <div
                      v-if="inboxSyncResult.data.syncStatus.lastSyncAt"
                    ...
                    >
                      <div>
                        ...
                        {{
                          new Date(
                            inboxSyncResult.data.syncStatus.lastSyncAt
                          ).toLocaleString()
                        }}
                      </div>
                    </div>
                ...
                <div
                  v-if="
                    inboxSyncResult.responseCode === 1 && inboxSyncResult.data
                  "
                  class="result-item error-item"
                >
                  <span class="result-label">Error:</span>
                  <span class="result-value error-text">{{
                    inboxSyncResult.data.error
                  }}</span>
                </div>
              </div>
            </div>
            ...
            <div v-if="inboxNotificationResult" class="test-result">
              ...
              <div class="result-content">
                ...
                <div class="result-item">
                  <span class="result-label">Message:</span>
                  <span class="result-value">{{
                    inboxNotificationResult.responseMessage
                  }}</span>
                </div>
                <div
                  v-if="
                ...
                >
                  ...
                  <el-tag type="info">{{
                    inboxNotificationResult.data.totalCount || 0
                  }}</el-tag>
                </div>
                <div
                  v-if="
                ...
                >
                  ...
                  <el-tag type="success">{{
                    inboxNotificationResult.data.itemCount
                  }}</el-tag>
                </div>
                <div
                  v-if="
                ...
                >
                  ...
                  <div class="notifications-preview">
                    <el-tag
                      v-for="(
                        notif, index
                      ) in inboxNotificationResult.data.notifications.slice(
                        0,
                        5
                      )"
                      :key="index"
                    ...
                    >
                      #{{ notif.id }}:
                      {{ notif.title?.substring(0, 30) || 'No title' }}...
                    </el-tag>
                    <span
                      v-if="
                        inboxNotificationResult.data.notifications.length > 5
                      "
                      class="more-ids"
                    >
                      ... and
                ...
                <div
                  v-if="
                    inboxNotificationResult.responseCode === 1 &&
                    inboxNotificationResult.data
                  "
                  class="result-item error-item"
                >
  ...
  const handleTestToken = async () => {
    ...
    try {
      ...
      if (result && result.isValid === true) {
        ...
        const errorMsg =
          result?.error || result?.errorCode || 'Token validation failed';
        ElNotification({
