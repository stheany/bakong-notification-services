...
import { templateApi } from '@/services/templateApi-v2'
...
const isCreatePage = computed(() => {
  ...
  return p.endsWith('/home') || p.endsWith('/v2/home')
})
...
const handleDeleteNotification = async (notificationId: number | string) => {
  try {
    if (USE_MOCK_DATA) {
      ...
      await templateApi.deleteNotification(Number(notificationId))
      ElNotification({
        title: 'Success',
        message: 'Notification deleted successfully',
        type: 'success',
...
const handlePublishNotification = async (notification: Notification) => {
  ...
  try {
    if (USE_MOCK_DATA) {
      ...
      // 0) Prevent re-send if already sent
      ...
      // 1) Call API
      const result = await notificationApi.sendNotification(Number(notificationId), undefined, true)

      // 2) Hard error only (backend envelope error)
      if (typeof result?.responseCode === 'number' && result.responseCode !== 0) {
        const errorMessage =
      ...
      // 4) Single source of truth for toast message/type
      const messageConfig = getNotificationMessage(
        result, // ✅ MUST be full envelope
        platformName,
        bakongPlatform,
        devicePlatformFormatted,
      )

      // 5) Always show toast (success/warning/error/info)
      ElNotification({
      ...
      // 6) Decide tab based on final outcome
      // - success or partial (warning) => published
      // - error/info (draft saved / failed) => draft
      const goToDraft = messageConfig.type === 'error' || messageConfig.type === 'info'
      activeTab.value = goToDraft ? 'draft' : 'published'

      // 7) Refresh list once (no duplicate logic below)
      cachedNotifications = null
      cacheTimestamp = 0
      clearCacheFromStorage()
      await fetchNotifications(true)
      ...
      // Optional: show error toast here instead of rethrow, your choice
      throw updateError
    }
  } catch (error: any) {
