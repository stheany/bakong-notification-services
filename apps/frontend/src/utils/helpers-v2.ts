...
//     (Array.isArray(data.failedUsers) && data.failedUsers) ||
//     (Array.isArray(data.usersInvalid) && data.usersInvalid) ||
//     (Array.isArray(data.invalidUsers) && data.invalidUsers) ||
//     []
...
//     typeof data.successfulCount === 'number'
//       ? data.successfulCount
//       : responseCode === 0
//       ? 1 // backend said success but didn’t send counts
//       : 0

//   const failedDueToInvalidTokens = data?.failedDueToInvalidTokens === true
//   const savedAsDraftNoUsers = data?.savedAsDraftNoUsers === true
...
//     if (!platform || platform.toUpperCase() === 'ALL') return ''
...
//   // ✅ NEW: if backend says error, show error message directly
...
//   // ---------------------------
//   if (savedAsDraftNoUsers && successfulCount === 0 && failedCount === 0) {
//     return {
//       title: 'Info',
...
//       failureReason =
//         ` All ${failedCount} user(s) have invalid or expired FCM tokens. ` +
//         `Users need to open the mobile app to refresh their tokens.`
//     } else {
//       failureReason = ` Failed to send to ${failedCount} user(s) , because this user(s) have invalid or expired FCM tokens.`
//     }

//     return {
//       title: 'Warning',
//       message:
//         `Failed to send the${bakongPlatformText} notification${devicePlatformText}.` +
//         `${failureReason} The notification has been saved as a draft.`,
//       type: 'warning',
//       duration: 8000,
//       dangerouslyUseHTMLString: true,
//     }
...
//       title: 'Success',
//       message:
//         `Notification${bakongPlatformText} sent${devicePlatformText}` +
//         ` to ${successfulCount} user(s) successfully. ` +
//         `Some users failed.`,
//       type: 'success',
//       duration: 4000,
//       dangerouslyUseHTMLString: true,
...
export const getNotificationMessage = (
  ...
  // ---------------------------
  const responseCode = result?.responseCode
  const responseMessage = result?.responseMessage || result?.message

  // Prefer envelope.data; fall back to result itself for legacy shapes
  const data = result?.data ?? result ?? {}
  ...
  const failedCount =
    typeof data.failedCount === 'number' ? data.failedCount : failedUsers.length

  const successfulCount =
    typeof data.successfulCount === 'number' ? data.successfulCount : 0
  ...
  // ---------------------------
  // CASE 0: Backend says error
  // ---------------------------
  if (typeof responseCode === 'number' && responseCode !== 0) {
    return {
      title: 'Error',
      message: responseMessage || 'Failed to send notification.',
      type: 'error',
      duration: 5000,
      dangerouslyUseHTMLString: false,
    }
  ...
  // ---------------------------
  // CASE 1: No users at all (saved as draft)
  // ---------------------------
  if (savedAsDraftNoUsers && successfulCount === 0 && failedCount === 0) {
    return {
      title: 'Info',
  ...
  if (successfulCount === 0 && failedCount > 0) {
    ...
    if (failedDueToInvalidTokens) {
      ...
      failureReason =
        ` Failed to send to ${failedCount} user(s) because they have invalid or expired FCM tokens.`
    }
  ...
  if (successfulCount > 0 && failedCount > 0) {
    return {
      ...
      message:
        ...
        ` But failed to send to ${failedCount} user(s).`,
      type: 'warning',
      duration: 5000,
      dangerouslyUseHTMLString: false,
    }
  ...
  if (successfulCount > 0 && failedCount === 0) {
    return {
      title: 'Success',
      message: responseMessage || 'Sent successfully.',
      type: 'success',
      duration: 3000,
      dangerouslyUseHTMLString: false,
    }
