...
import { categoryTypeApi, type CategoryTypeV2 } from '../services/categoryTypeApi-v2'
...
export const useCategoryTypesStore = defineStore('categoryTypes', () => {
  const categoryTypes = ref<CategoryTypeV2[]>([])
   ...
   */
  const loadCacheFromStorage = (): { categoryTypes: CategoryTypeV2[] | null; timestamp: number } => {
    try {
      ...
      if (cachedData && cachedTime) {
        ...
        if (now - timestamp < CACHE_TTL) {
          return {
            categoryTypes: JSON.parse(cachedData) as CategoryTypeV2[],
            timestamp,
          }
        } else {
   ...
   */
  const saveCacheToStorage = (types: CategoryTypeV2[], timestamp: number): void => {
    try {
   ...
   */
  const fetchCategoryTypes = async (forceRefresh = false): Promise<CategoryTypeV2[]> => {
   ...
   */
  const getCategoryTypeById = (id: number): CategoryTypeV2 | undefined => {
   ...
   */
  const getCategoryTypeByName = (name: string): CategoryTypeV2 | undefined => {
   ...
   */
  const addCategoryType = (categoryType: CategoryTypeV2): void => {
   ...
   */
  const updateCategoryType = (categoryType: CategoryTypeV2): void => {
