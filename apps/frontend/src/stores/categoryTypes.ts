...
export const useCategoryTypesStore = defineStore('categoryTypes', () => {
  ...
  const loadCacheFromStorage = (): {
    ...
    try {
      ...
      console.warn(
        'Failed to load category types cache from localStorage:',
        error
      );
      localStorage.removeItem(CACHE_STORAGE_KEY);
      localStorage.removeItem(CACHE_TIMESTAMP_KEY);
  ...
  const saveCacheToStorage = (
    ...
    try {
      ...
      console.warn(
        'Failed to save category types cache to localStorage:',
        error
      );
      try {
        ...
        console.error(
          'Could not save category types cache even after clearing:',
          e
        );
      }
    }
  ...
  const clearCacheFromStorage = (): void => {
    try {
      ...
      console.warn(
        'Failed to clear category types cache from localStorage:',
        error
      );
    }
  };
  ...
  const fetchCategoryTypes = async (
    ...
    try {
      ...
      error.value =
        err.response?.data?.message || 'Failed to fetch category types';
